        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-8 justify-center mb-16">
            {Array.from({ length: 6 }).map((_, i) => (
              <Skeleton key={i} className="partners-skeleton w-32 h-20 rounded-xl" />
            ))}
          </div>
          {showStats && (
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
              {Array.from({ length: 4 }).map((_, i) => (
                <Skeleton key={i} className="partners-skeleton h-24 rounded-xl" />
              ))}
            </div>
          )}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="text-center mb-12">
            ...
            >
              ...
              <Badge variant="secondary" className="partners-badge bg-lime-100 text-lime-700 hover:bg-lime-200 transition-colors">
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="flex-shrink-0">
                          <Card className="w-32 h-20 group cursor-pointer partners-logo-container">
                      ...
                      <TooltipContent side="bottom" className="partners-tooltip">
          ...
          <AnimatePresence>
            {isPaused && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 pause-indicator rounded-full p-2 z-20"
              >
        ...
        {showStats && (
          ...
          >
            <motion.div whileHover={{ y: -5 }} transition={{ duration: 0.2 }}>
              <Card className="text-center group partners-stats-card">
              ...
              <Card className="text-center group partners-stats-card">
                <CardContent className="p-6">
                  <div className="text-3xl font-bold text-lime-500 mb-2 group-hover:scale-110 transition-transform duration-300 flex items-center justify-center gap-1">
                    {partnersStats.averageRating}<span className="rating-star text-yellow-400">★</span>
                  </div>
